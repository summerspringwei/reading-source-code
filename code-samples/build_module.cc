...
class RelayBuildModule : public runtime::ModuleNode {
 ...
 protected:
  ...
  IRModule OptimizeImpl(IRModule relay_module) {
    // 1. 创建了一个Array，里面保存的是Pass
    Array<Pass> pass_seqs =
        GetPassPrefix(/*is_homogenous=*/
        config_->primitive_targets.size() == 1, /*is_vm=*/false);
    transform::PassContext pass_ctx = PassContext::Current();
    // Always plan devices so the remaining passes don't need to 
    // distinguish homogeneous vs hetrogenous execution.
    // 2. 把Pass加到pass_seqs中
    pass_seqs.push_back(transform::PlanDevices(config_));
    // Fuse the operations if it is needed.
    pass_seqs.push_back(transform::FuseOps());

    // 3. Create a sequential pass and perform optimizations.
